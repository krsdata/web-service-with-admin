...
class MonthlyReportController extends Controller {
    ...
    { 
        $page_title = 'Monthly Evaluation Report';  
        $page_action = 'Corporate monthly report'; 
        ...
        if (isset($search) && !empty($search)) { 
            ...
            $corporateProfile = CorporateProfile::with('user')->selectRaw('count(*) as count,company_name,id,company_url,created_at,userID')->orderBy('company_url','ASC')->groupBy('company_url')->Paginate($this->record_per_page);
        ...
        if($report=='pdf')
        {
           $pdf = PDF::loadView('packages::monthlyReport.corporate_profile_pdf', compact('corporateProfile', 'page_title', 'page_action'));
           return ($pdf->download('corporate_profile_report.pdf'));
        }
        /*---------Select from evaluation------*/        
        $y = InterviewRating::selectRaw('YEAR(created_at) as year')->lists('year')->toArray();
        if(count($y))
        {
            $year = array_unique($y);
        }else
        ...
        $report_yr =  ($request->get('year'))?$request->get('year'):date('Y');
       // dd($corporateProfile);
        /*--------------End----------------*/     
        return view('packages::monthlyReport.index', compact('report_yr','year','corporateProfile', 'page_title', 'page_action'));
    }
    ...
    */
    ...
    { 
        ...
        $users = User::whereHas('evaluatedCondidate',
            function($query) use ($user_ids) {
                $query->whereIn('interviewerID',$user_ids);
            }
        )->where(function($query)use($search){
            $query->where('first_name','LIKE',"%$search%");
            $query->orWhere('last_name','LIKE',"%$search%");
        })->orderBy('first_name','ASC')->paginate($this->record_per_page);
        ...
        $report_yr      =  ($request->get('year'))?$request->get('year'):date('Y'); 
        $download_url   =  $request->url().'?year='.$report_yr.'&report=pdf'; 
        $report = $request->get('report');
        if($report=='pdf')
        {
           $pdf = PDF::loadView('packages::monthlyReport.corporate_user_pdf',compact('year','report_yr','evaluation_by_month','month','download_url','request','users', 'page_title', 'page_action'));
           return ($pdf->download('corporate_user_report.pdf'));
        }
       
        
        return view('packages::monthlyReport.corporate_user', compact('year','report_yr','evaluation_by_month','month','download_url','request','users', 'page_title', 'page_action'));
